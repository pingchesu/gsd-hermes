...
이 인스톨러 기본값은 정식 런타임-모델 4경로 중 하나인 런타임 기본값 경로입니다. 즉, `resolve_model_ids: "omit"`은 누락 설정이 아니라 의도적인 동작입니다. 직접 런타임 지원이 없을 때만 `cross_ai_execution`을 명시적 외부 폴백으로 사용하세요.
