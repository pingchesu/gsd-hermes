...
- INST-02 chmod regression: `scripts/verify-fork-identity.cjs` and `sdk/dist/cli.js` are observed at mode `0o775` on the current tree; expected `0o755` per Phase 6 Plan 01 scaffolding spec and upstream #2525. `tests/hermes-install.test.cjs` asserts the strict mode and will fail until the files are re-chmod'd; a follow-on chmod-fix plan will restore `0o755` and investigate the install/build path that strips the bit.
- PROFILE-03 regression: `tests/bug-2516-inherit-model-execute-phase.test.cjs` subtest 5 (`init execute-phase emits empty executor_model when binding is inherit`) fails on the current tree. `resolveModelInternal` at `get-shit-done/bin/lib/core.cjs:1699` returns the literal string `'inherit'` for the inherit profile, but the init-context projection should emit `''` so the Task() call inherits from the orchestrator rather than receiving `model="inherit"` literally. Matches Plan 07-01 SUMMARY row 2 (legacy `'inherit'` / init `''`) — fix requires distinguishing legacy-vs-init projection contexts in `resolveModelInternal` OR updating the three `init.cjs` call sites (lines 113, 530, 1331) to use an init-aware resolver. Follow-on plan required; out of scope for Phase 7 Plan 03 verification.
