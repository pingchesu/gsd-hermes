...
// interim states where get-shit-done/bin/lib/*.cjs have not yet resolved their
...
Then re-run: npx gsd-hermes@latest
`);
    process.exit(1);
  }
}
...
const banner = '\n' +
  ...
  '  development system for Claude Code, OpenCode, Gemini, Kilo, Codex, Copilot, Antigravity, Cursor, Windsurf, Augment, Trae, Qwen Code, Cline, CodeBuddy and Hermes Agent by TÂCHES.\n';
...
if (hasHelp) {
  console.log(`  ${yellow}Usage:${reset} npx gsd-hermes [options]
  ...
  ${yellow}Options:${reset}
    ...
    ${cyan}--hermes${reset}                  Install for Hermes only (global or project-linked mode)
    ${cyan}--all${reset}                     Install for all runtimes
    ${cyan}-u, --uninstall${reset}           Uninstall GSD (remove all GSD files)
    ${cyan}--doctor${reset}                  Check GSD Hermes install health without changing files
  ...
  ${yellow}Examples:${reset}
    ...
    npx gsd-hermes --claude --global

    ${dim}# Install for Gemini globally${reset}
    npx gsd-hermes --gemini --global

    ${dim}# Install for Kilo globally${reset}
    npx gsd-hermes --kilo --global

    ${dim}# Install for Codex globally${reset}
    npx gsd-hermes --codex --global

    ${dim}# Install for Copilot globally${reset}
    npx gsd-hermes --copilot --global

    ${dim}# Install for Copilot locally${reset}
    npx gsd-hermes --copilot --local

    ${dim}# Install for Antigravity globally${reset}
    npx gsd-hermes --antigravity --global

    ${dim}# Install for Antigravity locally${reset}
    npx gsd-hermes --antigravity --local

    ${dim}# Install for Cursor globally${reset}
    npx gsd-hermes --cursor --global

    ${dim}# Install for Cursor locally${reset}
    npx gsd-hermes --cursor --local

    ${dim}# Install for Windsurf globally${reset}
    npx gsd-hermes --windsurf --global

    ${dim}# Install for Windsurf locally${reset}
    npx gsd-hermes --windsurf --local

    ${dim}# Install for Augment globally${reset}
    npx gsd-hermes --augment --global

    ${dim}# Install for Augment locally${reset}
    npx gsd-hermes --augment --local

    ${dim}# Install for Trae globally${reset}
    npx gsd-hermes --trae --global

    ${dim}# Install for Trae locally${reset}
    npx gsd-hermes --trae --local

    ${dim}# Install for Cline locally${reset}
    npx gsd-hermes --cline --local

    ${dim}# Install for CodeBuddy globally${reset}
    npx gsd-hermes --codebuddy --global

    ${dim}# Install for CodeBuddy locally${reset}
    npx gsd-hermes --codebuddy --local

    ${dim}# Install for Hermes globally under ~/.hermes${reset}
    ...
    npx gsd-hermes --kilo --global --config-dir ~/.kilo-work

    ${dim}# Install to current project only${reset}
    npx gsd-hermes --claude --local

    ${dim}# Uninstall GSD from Cursor globally${reset}
    npx gsd-hermes --cursor --global --uninstall
...
function readGsdRuntimeProfileResolver(targetDir = null) {
  // Lazy-require the runtime-aware tier resolver dependencies. Hoisting the
  // require here (rather than at module top) keeps bin/install.js loadable
  // when get-shit-done/bin/lib/{model-profiles,core}.cjs are themselves in an
  // unmerged state during a sync-branch interim (Phase 6 Plan 3 scope). When
  // the libs resolve cleanly, this require succeeds on first call; when they
  // don't, we return null and the caller falls back to the modelOverrides-only
  // code path that Hermes has used since v1.2.
  let GSD_MODEL_PROFILES;
  let gsdResolveTierEntry;
  try {
    const _gsdLibDir = path.join(__dirname, '..', 'get-shit-done', 'bin', 'lib');
    ({ MODEL_PROFILES: GSD_MODEL_PROFILES } = require(path.join(_gsdLibDir, 'model-profiles.cjs')));
    ({ resolveTierEntry: gsdResolveTierEntry } = require(path.join(_gsdLibDir, 'core.cjs')));
  } catch (err) {
    process.stderr.write(`gsd: warning — runtime-aware tier resolver unavailable (${err.message}); falling back to modelOverrides-only path\n`);
    return null;
  }
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  const converted = `${fm}\n${normalizedBody}`;
  return runtime === 'hermes' ? appendHermesRuntimeNote(converted) : converted;
}
...
/**
 * Copy GSD commands as Hermes skills.
 *
 * Hermes uses SKILL.md directories like Claude/Qwen skills, but keeping this
 * wrapper separate makes the Hermes adapter seam explicit for future syncs.
 *
 * @param {string} srcDir - Source commands directory
 * @param {string} skillsDir - Target skills directory
 * @param {string} prefix - Skill name prefix (e.g. 'gsd')
 ...
 * @param {boolean} isGlobal - Whether this is a global install
 */
function appendHermesRuntimeNote(content) {
  if (content.includes('## Hermes runtime compatibility')) {
    return content;
  }
...
- Use text-mode fallback when AskUserQuestion is unavailable.
- Use sequential inline execution when Task-style subagents are unavailable.
- Do not report full parity when a workflow needs a degraded path.
`;
}
...
function copyCommandsAsClaudeSkills(srcDir, skillsDir, prefix, pathPrefix, runtime = 'claude', isGlobal = false) {
  const cmdNames = readGsdCommandNames();
  const normalizedPrefix = prefix.endsWith('-') ? prefix : `${prefix}-`;
  const converter = (content, skillName) =>
    convertClaudeCommandToClaudeSkill(content, skillName, runtime, cmdNames);
  const staged = stageSkillsForRuntimeAsSkills(srcDir, { skills: '*' }, converter, normalizedPrefix);
  applyRuntimeContentRewritesInPlace(staged, runtime, pathPrefix);
  fs.mkdirSync(skillsDir, { recursive: true });
  _removeGsdEntries(skillsDir, { kind: 'skills', destSubpath: 'skills', prefix: normalizedPrefix });
  _copyStaged(staged, skillsDir, { kind: 'skills', destSubpath: 'skills', prefix: normalizedPrefix });
}
...
function hermesExternalDirBlockContains(lines, externalDirsIndex, normalizedPath) {
  const externalIndent = lineIndent(lines[externalDirsIndex]);
  const quotedPath = yamlQuote(normalizedPath);

  for (let i = externalDirsIndex + 1; i < lines.length; i++) {
    const line = lines[i];
    const trimmed = line.trim();
    if (trimmed && lineIndent(line) <= externalIndent) break;
    if (trimmed.includes(quotedPath) || trimmed.includes(normalizedPath)) {
      return true;
    }
  }
...
/**
 * Ensure Hermes scans a project-linked GSD skills directory.
 *
 * The mutation is intentionally narrow: only add one entry under
 * skills.external_dirs and preserve unrelated config text.
 *
 * @param {string} configPath - Hermes config.yaml path
 * @param {string} externalDir - Project skills directory to register
 * @returns {{added: boolean, path: string}} Whether a new entry was written
 */
function ensureHermesExternalDir(configPath, externalDir) {
  const normalizedPath = normalizeHermesExternalDir(externalDir);
  const quotedPath = yamlQuote(normalizedPath);
  ...
  const externalDirsIndex = lines.findIndex(line => /^\s*external_dirs:\s*$/.test(line));

  if (externalDirsIndex !== -1) {
    if (hermesExternalDirBlockContains(lines, externalDirsIndex, normalizedPath)) {
      return { added: false, path: normalizedPath };
    }
    ...
    while (insertIndex < lines.length) {
      ...
      if (line.trim() && lineIndent(line) <= externalIndent) break;
      insertIndex++;
    }
    lines.splice(insertIndex, 0, `${' '.repeat(externalIndent + 2)}- ${quotedPath}`);
  ...
  if (skillsIndex !== -1) {
    ...
    lines.splice(
      ...
      `${' '.repeat(skillsIndent + 4)}- ${quotedPath}`
    );
    fs.writeFileSync(configPath, lines.join('\n').replace(/\n*$/, '\n'));
    return { added: true, path: normalizedPath };
  }
  ...
  fs.writeFileSync(configPath, `${prefix}skills:\n  external_dirs:\n    - ${quotedPath}\n`);
  return { added: true, path: normalizedPath };
}

function removeHermesExternalDir(configPath, externalDir) {
  const normalizedPath = normalizeHermesExternalDir(externalDir);
  const quotedPath = yamlQuote(normalizedPath);
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (i <= externalDirsIndex) {
      nextLines.push(line);
      continue;
    }

    const trimmed = line.trim();
    if (trimmed && lineIndent(line) <= externalIndent) {
      nextLines.push(...lines.slice(i));
      break;
    }
  ...
  return { removed, path: normalizedPath };
}

function readHermesExternalDirs(configPath) {
  if (!fs.existsSync(configPath)) {
    return [];
  }
...
function addHermesDoctorFinding(findings, severity, filePath, message, fix) {
  findings.push({ severity, path: filePath, message, fix });
}

function scanHermesSkillFiles(skillsDir) {
...
function doctorHermesInstall({
  isGlobal = true,
  cwd = process.cwd(),
  homeDir = os.homedir(),
  explicitDir = explicitConfigDir,
} = {}) {
  const hermesRoot = explicitDir ? expandTilde(explicitDir) : path.join(homeDir, '.hermes');
  const targetDir = isGlobal ? hermesRoot : path.join(cwd, '.gsd-hermes');
  const mode = isGlobal ? 'global' : 'project-linked';
  ...
  if (!fs.existsSync(helpSkillPath)) {
    addHermesDoctorFinding(
      findings,
      'error',
      helpSkillPath,
      'Missing Hermes gsd-help skill',
      `Run npx gsd-hermes --hermes --${isGlobal ? 'global' : 'local'}`
    );
  } else {
    const helpSkill = fs.readFileSync(helpSkillPath, 'utf8');
    if (!/^name:\s+help\s*$/m.test(helpSkill)) {
      addHermesDoctorFinding(
        findings,
        'error',
        helpSkillPath,
        'Invalid Hermes gsd-help skill frontmatter',
        `Reinstall with npx gsd-hermes --hermes --${isGlobal ? 'global' : 'local'}`
      );
    }
  }
  ...
  if (!fs.existsSync(manifestPath)) {
    addHermesDoctorFinding(
      ...
      'warning',
      manifestPath,
      'Missing Hermes GSD file manifest',
      `Reinstall with npx gsd-hermes --hermes --${isGlobal ? 'global' : 'local'}`
    );
  }

  const claudeLeakPattern = /(?:~|\$HOME)\/\.claude\b|\.\/\.claude\b/;
  for (const skillPath of scanHermesSkillFiles(skillsDir)) {
    const content = fs.readFileSync(skillPath, 'utf8');
    if (claudeLeakPattern.test(content)) {
      addHermesDoctorFinding(
        findings,
        'error',
        skillPath,
        'Hermes skill contains executable Claude path leak',
        `Reinstall with npx gsd-hermes --hermes --${isGlobal ? 'global' : 'local'}`
      );
    }
  }

  const externalDirs = readHermesExternalDirs(configPath);
  if (!isGlobal) {
    const normalizedSkillsDir = normalizeHermesExternalDir(skillsDir);
    const matchCount = externalDirs.filter(entry => entry === normalizedSkillsDir).length;
    if (matchCount === 0) {
      addHermesDoctorFinding(
        findings,
        'error',
        configPath,
        'Missing project-linked Hermes external_dirs entry',
        'Run npx gsd-hermes --hermes --local'
      );
      ...
      addHermesDoctorFinding(
        ...
        'warning',
        configPath,
        'Duplicate project-linked Hermes external_dirs entry',
        'Remove duplicate entries from ~/.hermes/config.yaml'
      );
    }
  }
  ...
  for (const externalDir of externalDirs) {
    if (!fs.existsSync(externalDir)) {
      addHermesDoctorFinding(
        ...
        'warning',
        configPath,
        'Stale Hermes external_dirs entry',
        `Remove stale external_dirs entry ${externalDir}`
      );
    }
  }

  return { runtime: 'hermes', mode, targetDir, findings };
}
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  const targetDir = isGlobal
    ...
    : path.join(process.cwd(), isHermes ? '.gsd-hermes' : dirName);
  ...
  let hermesSkillCount = 0;
  if (isHermes) {
    hermesSkillCount = scanHermesSkillFiles(path.join(targetDir, 'skills')).length;
  }
  uninstallRuntimeArtifacts(runtime, targetDir, scope);
  removedCount++;
  if (isHermes && hermesSkillCount > 0) {
    console.log(`  ${green}✓${reset} Removed ${hermesSkillCount} Hermes skills`);
  }
  ...
  // 1a-hermes. Non-layout Hermes side-effect: project-linked skills.external_dirs cleanup.
  //     Hermes project-linked installs register ./.gsd-hermes/skills in ~/.hermes/config.yaml
  //     via skills.external_dirs; remove that entry on uninstall.
  if (isHermes && !isGlobal) {
    const hermesConfigPath = path.join(getGlobalDir('hermes', explicitConfigDir), 'config.yaml');
    const externalDirRemoval = removeHermesExternalDir(hermesConfigPath, path.join(targetDir, 'skills'));
    if (externalDirRemoval.removed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed project-linked Hermes skills.external_dirs entry`);
    }
  }
...
function writeManifest(configDir, runtime = 'claude', options = {}) {
  ...
  // Hermes namespaces GSD skills under skills/gsd/<stem>/SKILL.md. Other
  // skills-based runtimes keep the flat skills/gsd-<stem>/SKILL.md layout.
  const codexSkillsDir = isHermes ? path.join(configDir, 'skills', 'gsd') : path.join(configDir, 'skills');
  ...
  if ((isCodex || isCopilot || isAntigravity || isCursor || isWindsurf || isTrae || (!isOpencode && !isGemini)) && fs.existsSync(codexSkillsDir)) {
    ...
    if (isHermes) {
      ...
      if (fs.existsSync(descPath)) {
        manifest.files[`${codexSkillsManifestPrefix}DESCRIPTION.md`] = fileHash(descPath);
      }
    }
  }
  if (fs.existsSync(agentsDir)) {
...
function reportLocalPatches(configDir, runtime = 'claude') {
  ...
  if (meta.files && meta.files.length > 0) {
    ...
    if (reapplyCommand) {
      ...
      console.log('  Reapply the saved files manually from ' + cyan + PATCHES_DIR_NAME + '/' + reset + ' for Hermes installs.');
    }
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  const targetDir = isGlobal
    ? getGlobalDir(runtime, explicitConfigDir)
    : isHermes
      ? path.join(process.cwd(), '.gsd-hermes')
    : isCline
      ? process.cwd()
  ...
  if (isHermes && !isGlobal) {
    console.log(`  Installing for ${cyan}${runtimeLabel}${reset} in ${cyan}project-linked mode${reset} to ${cyan}${locationLabel}${reset}\n`);
  } else {
  ...
  if (_isSkillsRuntime) {
    ...
    // Hermes only: write DESCRIPTION.md for the nested gsd skill category after layout install
    if (isHermes) {
      const hermesSkillsRoot = path.join(targetDir, 'skills');
      const skillsDir = path.join(hermesSkillsRoot, 'gsd');
      writeHermesCategoryDescription(skillsDir);
      // Hermes project-linked mode: register the skills root in ~/.hermes/config.yaml
      // skills.external_dirs so Hermes discovers the nested gsd/ skill category.
      if (!isGlobal) {
        const configPath = path.join(getGlobalDir('hermes', explicitConfigDir), 'config.yaml');
        const result = ensureHermesExternalDir(configPath, hermesSkillsRoot);
        const status = result.added ? 'Registered' : 'Verified';
        console.log(`  ${green}✓${reset} ${status} project-linked Hermes skills in skills.external_dirs`);
      }
    }
    ...
    if (isHermes) {
      ...
      if (fs.existsSync(hermesGsdDir)) {
        const count = fs.readdirSync(hermesGsdDir, { withFileTypes: true })
          .filter(e => e.isDirectory() && fs.existsSync(path.join(hermesGsdDir, e.name, 'SKILL.md'))).length;
        if (count > 0) {
          console.log(`  ${green}✓${reset} Installed ${count} skills to skills/gsd/`);
        } else {
          failures.push('skills/gsd/<skill>/SKILL.md');
        }
      } else {
  ...
  if (isHermes) {
    // Hermes uses skills only here; project-linked config mutation happens during skill generation.
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (shouldInstallStatusline && !isOpencode && !isKilo && !isCodex && !isHermes && !isCopilot && !isCursor && !isWindsurf && !isTrae) {
...
`);
  ...
  if (isHermes) {
    const hermesRoot = getGlobalDir('hermes', explicitConfigDir).replace(os.homedir(), '~');
    if (!isGlobal) {
      const projectRoot = (configDir || path.join(process.cwd(), '.gsd-hermes')).replace(process.cwd(), '.');
      console.log(`
  ${green}Done!${reset} Hermes Agent configured in ${cyan}project-linked mode${reset} at ${cyan}${projectRoot}${reset}.

  Hermes will discover these project skills through ${cyan}${hermesRoot}/config.yaml${reset} ${cyan}skills.external_dirs${reset}.
  Restart or reload Hermes if needed, then run ${cyan}/gsd-help${reset} to verify command discovery.
  GSD defaults remain managed in ${cyan}~/.gsd/defaults.json${reset} with ${cyan}resolve_model_ids: "omit"${reset} for non-Claude runtimes.
...
`);
    ...
    console.log(`
  ${green}Done!${reset} Hermes Agent configured in ${cyan}global mode${reset} at ${cyan}${hermesRoot}${reset}.

  GSD defaults remain managed in ${cyan}~/.gsd/defaults.json${reset} with ${cyan}resolve_model_ids: "omit"${reset} for non-Claude runtimes.
  Restart Hermes and run ${cyan}/gsd-help${reset} to verify command discovery.
...
function buildRuntimePromptText() {
  ...
  ${cyan}10${reset}) Hermes Agent ${dim}(~/.hermes global; ./.gsd-hermes project-linked)${reset}
...
function promptLocation(runtimes) {
  ...
  const localExamples = runtimes.map(r => r === 'hermes' ? './.gsd-hermes (project-linked)' : `./${getDirName(r)}`).join(', ');
...
if (!process.env.GSD_TEST_MODE) {
  if (hasSkillsRoot) {
    ...
    // Hermes nests GSD skills under skills/gsd when using upstream-style global category installs.
    // gsd-hermes project-linked installs still write ./.gsd-hermes/skills and register skills.external_dirs.
    ...
    if (!selectedRuntimes.includes('hermes')) {
      console.error(`  ${yellow}--doctor currently supports --hermes${reset}`);
      process.exit(1);
    }

    const result = doctorHermesInstall({ isGlobal: hasGlobal || !hasLocal });
    console.log(`  Hermes doctor: ${result.mode}`);
    if (result.findings.length === 0) {
      console.log(`  ${green}✓${reset} Hermes doctor: no issues found`);
    } else {
      for (const finding of result.findings) {
        console.log(`  ${finding.severity}: ${finding.message} [${finding.path}] -> ${finding.fix}`);
      }
    }
