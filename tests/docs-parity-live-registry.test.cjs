...
function isReleaseDoc(filePath) {
  ...
  return (
    (path.basename(filePath).startsWith('RELEASE-') && filePath.endsWith('.md')) ||
    normalized.includes('/docs/releases/') ||
    normalized.includes('/docs/sync-logs/')
  );
}
...
const INTERNAL_COMPONENT_SLUGS = new Set([
  ...
  // gsd-hermes — downstream fork / npm package / repository name. Docs refer
  // to paths such as github.com/pingchesu/gsd-hermes and install commands such
  // as `npm install -g gsd-hermes`; it is not a user-typable slash command.
  'hermes',

  // gsd-sdk — CLI binary name referenced in SDK diagnostics and release docs.
  // It is invoked as a shell command, not as a slash command in the GSD registry.
  'sdk',
]);
