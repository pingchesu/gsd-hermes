...
const { loadConfig, isGitIgnored, normalizePhaseName, comparePhaseNum, getArchivedPhaseDirs, generateSlugInternal, getMilestoneInfo, getMilestonePhaseFilter, resolveModelInternal, resolveModelBindingInternal, resolveReasoningEffortInternal, stripShippedMilestones, extractCurrentMilestone, toPosixPath, output, error, findPhaseInternal, extractOneLinerFromBody, getRoadmapPhaseInternal } = require('./core.cjs');
...
function cmdResolveModel(cwd, agentType, raw) {
  ...
  const binding = resolveModelBindingInternal(cwd, agentType);
  ...
  const result = agentModels
    ...
    : {
        ...
        rejection_reason: binding.rejectionReason || 'unknown-agent',
        suggested_fix: binding.suggestedFix,
      };
