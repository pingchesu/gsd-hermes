...
function resolveModelInternal(cwd, agentType, options = {}) {
  ...
  // profile=inherit (#3030 CR Major), while init-context keeps omitting
  // inherited model IDs for runtime defaults.
  if (tier === 'inherit') return options.initContext ? '' : 'inherit';
